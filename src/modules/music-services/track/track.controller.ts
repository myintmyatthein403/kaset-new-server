...
export class TrackController {
  constructor(private readonly baseService: TrackService) { }
  ...
  @ApiQuery({
    name: 'filters',
    required: false,
    description: 'JSON object for filtering entities (e.g., {"name": "Test"})',
    type: 'string',
  })
  @ApiQuery({
    name: 'page',
  ...
  @ApiResponse({
    status: 200,
    description: 'Paginated and filtered list of entities',
  })
  async findAll(
  ...
  async update(
    ...
    return this.baseService.updateTrack(id, data);
  }

  @UseGuards(JwtOrApiKeyGuard)
  @Get('/slug/:name')
  async findTrackBySlug(@Param('name') name: string) {
    return this.baseService.fndWithSlug(name);
  }
